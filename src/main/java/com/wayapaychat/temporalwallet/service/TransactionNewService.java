...
//                    res.setCustomerWalletId(userAccount.getId());
             ...
             // Handle Credit USER Account
                if (command.equals("CREDIT")) {
                    ...
                    transactionRepository.save(transaction);
                    Double total = userAccount.getBalance() + request.getAmount();
                    System.out.println(":::Total:::"+total);
                    userAccount.setBalance(total);
                    userAccount.setLagerBalance(total);
                    List<Transactions> transactionList = userAccount.getTransactions();
                    transactionList.add(transaction);
                    accountRepository.save(userAccount);
...
//                    res.setCustomerWalletId(userAccount.getId());
			...
			return accountRepository.findById(walletId).map(account -> {
				...
				return new ApiResponse.Builder<>()
					...
					new ApiResponse.Builder<>()
			                .setStatus(false)
			                .setCode(ApiResponse.Code.NOT_FOUND)
			                .setMessage("Wallet Id provided not found")
			                .build()
					);
		} catch (Exception e) {
			log.info(e.getMessage());
			...
			return accountRepository.findByAccountNo(accountNumber).map(account -> {
				...
				return new ApiResponse.Builder<>()
					...
					new ApiResponse.Builder<>()
			                .setStatus(false)
			                .setCode(ApiResponse.Code.NOT_FOUND)
			                .setMessage("Account Number provided not found")
			                .build()
					);
		} catch (Exception e) {
			log.info(e.getMessage());
    ...
    //THIS IS THE METHOD RESPONSIBLE FOR HANDLING THE TRANSACTION
    //CREATED ON 6/24/2021 AFTER SEVERAL MODIFICATION ALL OTHER TRANSACTION
    //SHOULD BE IGNORED.
    @Transactional
    public ApiResponse makeWalletTransaction(MifosTransactionPojo transactionPojo, String command) {
    	try {
    		if(transactionPojo.getAmount() < 1) {
    			return new ApiResponse.Builder<>()
		                .setStatus(false)
		                .setCode(ApiResponse.Code.BAD_CREDENTIALS)
		                .setMessage("Wrong Amount, amount cannot be 0 or negetive")
		                .build();
    		}
    		//Retrieve Account
			Optional<Accounts> beneficiaryAccount = accountRepository.findById(transactionPojo.getBeneficiaryWalletId());
			Optional<Accounts> senderAccount = accountRepository.findById(transactionPojo.getCustomerWalletid());
			Optional<Accounts> wayaAccount = accountRepository.findByAccountNo(WAYA_SETTLEMENT_ACCOUNT_NO);
			if(beneficiaryAccount.isPresent() && senderAccount.isPresent() && wayaAccount.isPresent()) {
				String ref = randomGenerators.generateAlphanumeric(12);
				//HANDLE CREDIT
				if(command.equals("CREDIT")) {
					
					if(transactionPojo.getAmount() > senderAccount.get().getBalance()) {
		    			return new ApiResponse.Builder<>()
				                .setStatus(false)
				                .setCode(ApiResponse.Code.BAD_CREDENTIALS)
                    ...
                    transactionRepository.save(transaction);
                    beneficiaryAccount.get().setBalance(beneficiaryAccount.get().getBalance() + transactionPojo.getAmount());
                    beneficiaryAccount.get().setLagerBalance(beneficiaryAccount.get().getBalance() + transactionPojo.getAmount());
                    List<Transactions> transactionList = beneficiaryAccount.get().getTransactions();
                    transactionList.add(transaction);
					
					beneficiaryAccount.get().setBalance(beneficiaryAccount.get().getBalance() + transactionPojo.getAmount());
//					beneficiaryAccount.get().se
                  ...
                  //CREATE TRANSACTION HISTORY
                    ...
                    transactionRepository.save(transaction2);
                    senderAccount.get().setBalance(senderAccount.get().getBalance() - transactionPojo.getAmount());
                    senderAccount.get().setLagerBalance(senderAccount.get().getBalance() - transactionPojo.getAmount());
                    List<Transactions> transactionList2 = senderAccount.get().getTransactions();
                    transactionList2.add(transaction2);
					senderAccount.get().setBalance(senderAccount.get().getBalance() - transactionPojo.getAmount());
                    ...
                    wayaAccount.get().setBalance(wayaAccount.get().getBalance() + transactionPojo.getAmount());
                    List<Transactions> transactionList4 = wayaAccount.get().getTransactions();
...
//                    res.setCustomerWalletId(userAccount.getId());
				...
				if(command.equals("DEBIT")) {
                    ...
                    senderAccount.get().setBalance(senderAccount.get().getBalance() - transactionPojo.getAmount());
                    senderAccount.get().setLagerBalance(senderAccount.get().getBalance() - transactionPojo.getAmount());
                    List<Transactions> transactionList = senderAccount.get().getTransactions();
                    transactionList.add(transaction);
					senderAccount.get().setBalance(senderAccount.get().getBalance() - transactionPojo.getAmount());
					accountRepository.save(senderAccount.get());
